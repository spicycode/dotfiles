...
require("lazyload").on_vim_enter(function()
	...
	local function build_content()
		...
		local function render_plugin(p, icon, hl_group)
			...
			local unreleased_count_str = unreleased_count > 0 and string.format("  +%d unreleased", unreleased_count)
				or ""
			...
			if state.expanded[name] then
				...
				if unreleased_commits and #unreleased_commits > 0 then
					add(
						string.format("     +%d unreleased commit(s) on main", #unreleased_commits),
						"PackUiUpdateAvailable"
					)
					line_to_plugin[#lines] = name
					add("")
				end
